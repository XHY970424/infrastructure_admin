...
import { imgBase64Upload } from "@/api/content/contContents";
export default {
  name: "kindeditor",
  data() {
    return {
  ...
  methods: {
    ...
    async selectedFile($event) {
      ...
      let uploadBase64 = await this.arrayBufferToBase64(file);
      setTimeout(() => {
        imgBase64Upload(uploadBase64.result).then((res) => {
          if (res.code == 200) {
            this.editor.insertHtml(`<img src="${res.url}" alt="" />`);
            setTimeout(() => {
              console.log(this.outContent, this.content);
            }, 500);
            // this.$forceUpdate();
          } else {
