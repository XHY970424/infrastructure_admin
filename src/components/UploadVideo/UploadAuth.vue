      ...
      createUploader(type) {
        ...
        let uploader = new AliyunUpload.Vod({
          ...
          addFileSuccess: function(uploadInfo) {
            ...
            self.$emit('changeTxt','添加文件成功, 等待上传...')

            let url = URL.createObjectURL(uploadInfo.file)
            ...
            audioElement.addEventListener('loadedmetadata', () => {
              ...
              self.$emit('chanegePlayTime', parseInt(playTime))
              // console.log('时长：' + parseInt(playTime))
            })
          ...
          onUploadstarted: function(uploadInfo) {
            ...
            if (!uploadInfo.videoId) {
              ...
              createUploadVideo(queryParams).then(response => {
              ...
              refreshUploadVideo(queryParams).then(response => {
                let uploadAuth = response.data.uploadAuth
                let uploadAddress = response.data.uploadAddress
                let videoId = response.data.videoId
                uploader.setUploadAuthAndAddress(uploadInfo, uploadAuth, uploadAddress, videoId)
          ...
          onUploadSucceed: function(uploadInfo) {
            ...
            getPlayInfo(queryParams).then(response => {
              self.$emit('changeVideo', response.data.playUrl)
            })
            // self.statusText = '文件上传成功!'
            self.$emit('changeTxt','文件上传成功')
          },
          // 文件上传失败
          onUploadFailed: function(uploadInfo, code, message) {
            // console.log('onUploadFailed: file:' + uploadInfo.file.name + ',code:' + code + ', message:' + message)
            // self.statusText = '文件上传失败!'
            self.$emit('changeTxt','文件上传失败')
          },
          // 取消文件上传
          onUploadCanceled: function(uploadInfo, code, message) {
            // console.log('Canceled file: ' + uploadInfo.file.name + ', code: ' + code + ', message:' + message)
            // self.statusText = '文件已暂停上传'
            self.$emit('changeTxt','文件已暂停上传')
          },
          ...
          onUploadProgress: function(uploadInfo, totalSize, progress) {
            ...
            self.$emit('changeTxt','文件上传中...')
          },
          // 上传凭证超时
          onUploadTokenExpired: function(uploadInfo) {
            // console.log('进入上传凭证超时方法')
            ...
            self.$emit('changeTxt','文件超时...')
          },
          // 全部文件上传结束
          onUploadEnd: function(uploadInfo) {
            console.log('onUploadEnd: uploaded all the files')
            // self.statusText = '文件上传完毕'
            self.$emit('changeTxt','文件上传完毕')
          }
        })
        return uploader
      }
