<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      ...
      <el-form-item>
        ...
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['eycode:specialcategory:add']"
        >新增
        </el-button>
      </el-form-item>
    </el-form>
    ...
    >
      <el-table-column label="专题分类名" header-align="center" prop="categoryName" width="300px"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['eycode:specialcategory:edit']"
          >修改
          </el-button>
          <el-button
            size="mini"
            ...
            v-hasPermi="['eycode:specialcategory:remove']"
          >删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <!-- 添加或修改专题分类 对话框 -->
    <SpecialCategory @cancelCourseCategory="open=false" @getList="getList" :categoryTitle="title" :categoryOpen="open"
                    :dataForm="form"
                    v-if="open"></SpecialCategory>
...
<script>
  ...
  } from '@/api/course/specialcategory'
  ...
  export default {
    ...
    components: { Treeselect,SpecialCategory },
    data() {
      return {
        //是否上架数据字典
        isPutwayOptions: [],
    ...
    methods: {
      ...
      getList() {
        this.loading = true
        listSpecialcategory(this.queryParams).then(response => {
      ...
      handleUpdate(row) {
        ...
        getSpecialcategory(row.id).then(response => {
      ...
      handleDelete(row) {
        this.$confirm('是否确认删除专题分类 编号为"' + row.id + '"的数据项?', '警告', {
          ...
          return delSpecialcategory(row.id)
        }).then(() => {
          this.getList()
