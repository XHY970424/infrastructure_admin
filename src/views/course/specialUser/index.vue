<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      ...
      <el-form-item label="所属班级" prop="classId">
        <el-select v-model="queryParams.classId" clearable>
          <el-option v-for="item in classData" :label="item.className" :value="item.cid" />
        </el-select>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['eycode:specialuser:export']"
        >导出</el-button>
      </el-col>
    </el-row>
    ...
    <el-table v-loading="loading"  border :data="userList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="ID" align="center" prop="id" width="60px" fixed="left" />
      <el-table-column label="专题标题" align="center" prop="specialName" width="100" fixed="left" :show-overflow-tooltip="true"/>
      <el-table-column label="学员姓名" align="center" prop="nickName" width="100" fixed="left" :show-overflow-tooltip="true"/>
      <el-table-column label="身份证号" align="center" prop="idCard" width="180" fixed="left" :show-overflow-tooltip="true"/>
      <el-table-column label="学员手机号码" align="center" prop="phonenumber" fixed="left" width="140" :show-overflow-tooltip="true"/>
      <el-table-column label="班级名称" align="center" prop="className" />
      <el-table-column label="总时长(分)(不包含考试)" align="center" prop="allTime" >
        <template scope="scope">
          <span>{{ (scope.row.allTime / 60).toFixed(2) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="已学时长(秒)(不包含考试)" align="center" prop="alreadyTime" />
      <el-table-column label="所属单位" align="center" prop="companyName" />
        <!-- <template slot-scope="scope">
          <span v-if="scope.row.companyName != null "> {{scope.row.unitName}} </span>
          <span v-if="scope.row.companyName == null "> {{scope.row.unitName}} </span>
        </template>
      </el-table-column> -->
      ...
      <el-table-column label="完成状态" align="center" width="100">
        <template slot-scope="scope">
          <span v-if="scope.row.studyStatus == 0"> 未学完 </span>
          <span v-if="scope.row.studyStatus == 1"> 已学完 </span>
        </template>
      </el-table-column>
      <el-table-column label="完成时间" align="center" prop="finishTime" width="130"/>
...
<script>
  import { listSpecialUser, getClassBySpecial, exportSpecialUser } from "@/api/course/specialUser";
  ...
  export default {
    ...
    created() {
      this.specialId = this.$route.params && this.$route.params.specialId;
      if(this.specialId){
        this.queryParams.specialId = this.specialId;
      }
      this.getList();
      // 获取班级
      getClassBySpecial(this.specialId).then(res => {
    ...
    methods: {
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出所有课程人员管理关联数据项?', "警告", {
          ...
          return exportSpecialUser(queryParams);
        }).then(response => {
