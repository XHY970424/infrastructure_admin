<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" label-width="68px">
      <el-form-item label="面授课程" prop="courseTitle">
        <el-input
    ...
    <el-table v-loading="loading" :data="courseList" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="面授课程名称" align="center" prop="courseTitle" />
    ...
    <el-dialog :title="title" :close-on-click-modal="false" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="面授课程" prop="courseId">
...
<script>
import { listCourse, getCourse, delCourse, addCourse, updateCourse, exportCourse } from "@/api/cms/mscourse";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除面授课程栏目数据编号为"' + indexCourseIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有面授课程栏目数据数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
