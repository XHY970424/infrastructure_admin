<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="specialList" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="专题" align="center" prop="specialTitle" />
...
<script>
import { listSpecial, getSpecial, delSpecial, addSpecial, updateSpecial, exportSpecial } from "@/api/cms/special";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除专题栏目数据编号为"' + indexSpecialIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有专题栏目数据数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
