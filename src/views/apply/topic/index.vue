<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="发证机构" align="center" prop="authorityName" />
      <el-table-column label="课题价格" align="center" prop="price" />
      <el-table-column label="招生价" align="center" prop="admissionPrice" />
      <el-table-column label="期次数" align="center" prop="periodNum" />
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['apply:topic:recover']"
            >恢复
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="发证机构" prop="authorityIds">
          <el-row>
            <el-col :span="12">
              <el-select
        ...
        <el-row>
          <el-col :span="16">
            <el-form-item label="课题价格" prop="price">
              <el-input-number
                v-model="form.price"
                label="请输入价格"
                :min="0"
          ...
          <el-col :span="6" style="position: relative; top: 10px; color: gray">
            <div>设置0元时免费</div>
          </el-col>
        </el-row>
        <el-form-item label="招生价" prop="admissionPrice">
          <el-input-number
            v-model="form.admissionPrice"
            label="请输入招生价"
            :min="0"
            :step="0.01"
            step-strictly
        ...
        <el-form-item label="报名表配置继承报名表" prop="extendApplyForm">
          <el-radio-group v-model="form.extendApplyForm">
            <el-radio
              v-for="dict in extendApplyFormOptions"
              :key="dict.dictValue"
              :label="dict.dictValue"
              >{{ dict.dictLabel }}
            </el-radio>
        ...
        <el-form-item
          label="报名条件配置继承报名表"
          prop="extendApplyCondition"
        >
          <el-radio-group v-model="form.extendApplyCondition">
            <el-radio
              v-for="dict in extendApplyFormOptions"
              :key="dict.dictValue"
              :label="dict.dictValue"
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="12">
            <el-form-item label="课题名称：">{{ info.topicName }}</el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="课题价格：">￥{{ info.price }}</el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="招生价："
              >￥{{ info.admissionPrice }}
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
} from "@/api/apply/topic";
import { addCertifyingAuthority } from "@/api/apply/certifyingAuthority";
import { listCertifyingAuthority } from "@/api/apply/certifyingAuthority";
import { queryAllGroupByColumn } from "@/api/apply/project";
...
export default {
    ...
    return {
      ...
      porp: {
        callback: (formData, resolve) => {
          addCertifyingAuthority(formData).then((response) => {
      ...
      rules: {
        ...
        extendApplyForm: [
          {
            required: true,
            message: "报名表配置继承报名表不能为空",
            trigger: "blur",
          },
        ],
        extendApplyCondition: [
          {
            required: true,
            message: "报名条件配置继承报名表不能为空",
            trigger: "blur",
          },
        ],
        authorityName: [
      ...
      handler: function (val, oldVal) {
        if (
          ...
          this.queryParams.projectName =
            this.$route.query && this.$route.query.projectName;
          this.queryParams.projectId =
            this.$route.query && this.$route.query.projectId;
          this.getAuthority();
          this.getList();
        }
      },
  ...
  methods: {
    ...
    //选择字段
    firldChange(val, fIndex, cIndex) {//fIndex==-1为条件，否则为条件数组 val为选中值的主键 fIndex==-1时cIndex为条件的索引 否则fIndex为条件数组的索引 cIndex为条件数组里的条件索引
      let index = this.applyOptions.findIndex((item) => item.fieldId == val);
      let fieldDictId = this.applyOptions[index].formDictId;
      if (fIndex != -1) {
        ...
        if (this.applyOptions[index].inputType.indexOf("lazy_tree") != -1) {//是否为联动
          let inputType = this.applyOptions[index].inputType.split("_");
          this.condition.child[fIndex].conditionList[cIndex].selectNum =
            inputType[2];//几级联动渲染几个选择框
          this.condition.child[fIndex].conditionList[cIndex].isEcho = false;//是否是已填写状态
        }
        this.condition.child[fIndex].conditionList[cIndex].compareType = "";//条件
        this.condition.child[fIndex].conditionList[cIndex].selectItem =
          this.applyOptions[index].inputType == "checkbox" ? [] : "";//值
        this.condition.child[fIndex].conditionList[cIndex].fieldDictId =
          fieldDictId;//获取动态表单选项字典-参数
      } else {
        ...
        this.condition.conditionList[cIndex].typeKey =
          this.applyOptions[index].typeKey;
        if (this.applyOptions[index].inputType.indexOf("lazy_tree") != -1) {//判断是否为联动
          let inputType = this.applyOptions[index].inputType.split("_");
          this.condition.conditionList[cIndex].selectNum = inputType[2];//几级联动渲染几个选择框
          this.condition.conditionList[cIndex].isEcho = false;//是否是已填写状态
        }
        this.condition.conditionList[cIndex].compareType = "";//条件
        this.condition.conditionList[cIndex].selectItem =
          this.applyOptions[index].inputType == "checkbox" ? [] : "";//值
        this.condition.conditionList[cIndex].fieldDictId = fieldDictId;//获取动态表单选项字典-参数
      }
      if (this.treeInputType.includes(this.applyOptions[index].inputType)) {//如果inputType包含treeInputType数组里（满足条件）
        this.recursionTree(fieldDictId, 0, fIndex, cIndex);//获取动态表单选项字典
      }
    },
    //获取动态表单选项字典
    ...
    submitCondition() {
      ...
      // }
      if (this.extend == 1) {
        this.msgError("不可以编辑继承于项目的报名条件配置");
        return;
      }
    ...
    //获取报名条件配置
    conditionSet(row) {
      if (row.extendApplyCondition == 1) {
        this.msgError("不可以编辑继承于项目的报名条件配置");
        return;
      }
      const extendApplyForm = row.extendApplyForm;
      ...
      this.condition.topicId = topicId;
      Promise.all([
        topicConditionField(
          extendApplyForm == 1 ? projectId : topicId,
          extendApplyForm
        ),
        queryFormCondition(topicId, this.queryParams.projectId),
      ]).then((res) => {
    ...
    submitSet() {
      ...
      saveTopicApply({ topicId: this.topicId, data }).then((res) => {
    ...
    //获取报名表配置列表
    applySet(row) {
      if (row.extendApplyForm == 1) {
        this.msgError("不可以编辑继承于项目的报名表配置");
        return;
      }
      const topicId = row.topicId;
      this.topicId = topicId;
      Promise.all([topicForm(topicId), queryAllGroupByColumn()]).then((res) => {
        let use = res[0].data;
        let list = res[1].data;
        list.forEach((item) => {
          item.fieldList.forEach((ite) => {
    ...
    goPeriod(row) {
      ...
      let topicName = row.topicName;
      let projectName = this.queryParams.projectName;
      this.$router.push({
        path: "/apply/period",
        query: { topicId, projectName, topicName },
      });
    },
    /**查看详情 */
    handleInfo(row) {
    ...
    handleStatusChange(row) {
      ...
      this.$confirm(
        ...
        .then(function () {
          return updateStatus({ ids: [row.topicId], status: row.status });
        })
        .then(() => {
    ...
    /** 查询发证机构列表 */
    getAuthority() {
      listCertifyingAuthority().then((res) => {
        this.authorityOptions = res.rows;
      });
    },
    /** 查询报名课题列表 */
    getList() {
    ...
    handleUpdate(row) {
      ...
      getTopic(topicId).then((response) => {
        ...
        this.isAdd = false;
        if (this.form.authorityIds.indexOf(",") != -1) {
          this.form.authorityIds = this.form.authorityIds.split(",");
        } else {
          this.form.authorityIds = [this.form.authorityIds];
        }
        this.open = true;
        this.addCancel();
        this.title = "修改报名课题";
    ...
    submitForm: function () {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          this.form.authorityBoList = [];
          let authorityBoList = this.authorityOptions.filter(
            (item) => this.form.authorityIds.indexOf(item.authorityId) != -1
          );
          authorityBoList.forEach((ite) => {
            this.form.authorityBoList.push({
              authorityId: ite.authorityId,
              authorityName: ite.authorityName,
            });
          });
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有报名课题数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
