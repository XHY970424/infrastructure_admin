<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="分销企业数" align="center" prop="companyNum"/>
      <el-table-column label="分销预报名人数" align="center" prop="applyNum"/>
...
<script>
  import { distributionList } from '@/api/apply/schedulePeriod'
  ...
  export default {
      ...
      $route: {
        handler: function(val, oldVal) {
          if (
            ...
            this.queryParams.history = this.$route.query && this.$route.query.history?this.$route.query.history:undefined
            this.getList()
          }
        },
        // 深度观察监听
    ...
    methods: {
      ...
      goUserInfo(row) {
        ...
        let history = this.queryParams.history?1:''
        this.$router.push({
          path: '/train/distributionUserInfo',
          query: { projectName, periodId, topicName,periodName,history }
        })
      },
      /** 状态判断 */
      ...
      getList() {
        this.loading = true
        distributionList(this.queryParams).then((response) => {
