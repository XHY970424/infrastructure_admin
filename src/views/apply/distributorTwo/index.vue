    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['apply:distributorTwo:add']"
        >新增分销商(二级)</el-button>
      </el-col>
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="single"-->
<!--          @click="handleUpdate"-->
<!--          v-hasPermi="['apply:distributorTwo:edit']"-->
<!--        >修改</el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="multiple"-->
<!--          @click="handleDelete"-->
<!--          v-hasPermi="['apply:distributorTwo:remove']"-->
<!--        >删除</el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          size="mini"-->
<!--          @click="handleExport"-->
<!--          v-hasPermi="['apply:distributorTwo:export']"-->
<!--        >导出</el-button>-->
<!--      </el-col>-->
    </el-row>

    <el-table v-loading="loading" :data="distributorTwoList" @selection-change="handleSelectionChange" border>
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="分销商名称" align="center" prop="distributorName" />
      <el-table-column label="用户名" align="center" prop="nickName" />
      <el-table-column label="手机号" align="center" prop="phonenumber" />
      <el-table-column label="管理课题数" align="center" prop="topicNum" />
...
<!--          >删除</el-button>-->
    ...
    <!-- 分销商(二级)详情对话框 -->
    <el-dialog title="分销商(二级)详情" :visible.sync="detailOpen" width="1000px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="分销商名" prop="distributorName">
...
<script>
import { listDistributorTwo, getDistributorTwo, delDistributorTwo, addDistributorTwo, updateDistributorTwo, exportDistributorTwo,updateStatus } from "@/api/apply/distributorTwo";
...
export default {
  ...
  methods: {
    /** 详情 */
    handleDetail(row){
      getDistributorTwo(row.distributorId).then(response => {
    ...
    getList() {
      this.loading = true;
      listDistributorTwo(this.queryParams).then(response => {
        this.distributorTwoList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getDistributorTwo(distributorId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改分销商(二级)";
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.distributorId != undefined) {
            updateDistributorTwo(this.form).then(response => {
            ...
            addDistributorTwo(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除分销商(二级)编号为"' + distributorIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delDistributorTwo(distributorIds);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有分销商(二级)数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportDistributorTwo(queryParams);
        }).then(response => {
